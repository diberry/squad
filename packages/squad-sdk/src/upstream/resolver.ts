 ...
 */
export function readUpstreamConfig(squadDir: string, storage: StorageProvider = new FSStorageProvider()): UpstreamConfig | null {
 ...
 */
function findSquadDir(sourcePath: string, storage: StorageProvider): string | null {
 ...
 */
function readSkills(squadDir: string, storage: StorageProvider): Array<{ name: string; content: string }> {
  ...
  try {
    for (const entry of storage.listSync(source.dir)) {
      ...
      if (storage.existsSync(skillFile)) {
        ...
        const content = storage.readSync(skillFile);
 ...
 */
function readOptionalFile(filePath: string, storage: StorageProvider): string | null {
 ...
 */
function readOptionalJson(filePath: string, storage: StorageProvider): Record<string, unknown> | null {
 ...
 */
function resolveFromSquadDir(name: string, type: 'local' | 'git' | 'export', upstreamSquadDir: string, storage: StorageProvider): ResolvedUpstream {
  return {
    name,
    type,
 ...
 */
function resolveFromExport(name: string, exportPath: string, storage: StorageProvider): ResolvedUpstream {
 ...
 */
export function resolveUpstreams(squadDir: string, storage: StorageProvider = new FSStorageProvider()): UpstreamResolution | null {
