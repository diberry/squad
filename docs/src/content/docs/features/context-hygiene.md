...
**What it does:** Summarizes accumulated work in `.squad/` team state files into smaller, more efficient memory files. This compresses your persistent team knowledge — agent histories and decisions — across multiple sessions. Old logs are pruned (deleted) after a short retention window rather than archived.
