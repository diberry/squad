...
**PR revision protocol and test coverage gaps (PR #431 revision) — 2026-03-16**
- When revising a blocked PR after review rejection, the reviewer's feedback takes priority even if the original author disagrees with the strictness
- Test scanner for spawn context pollution must scan ALL files loaded at spawn: charter.md, squad.agent.md, AND history.md files
- History files documenting work done can accidentally create spawn context leaks if they mention specific paths — use generic descriptions instead
- Stress tests should reflect mature scale: 500 files (2MB+) is more realistic than 100 files for a knowledge library
- CI integration prevents regressions: GitHub Actions should run validation on any PR touching the protected area
- Frontmatter validation needs edge case handling: README.md files in knowledge directories are indexes, not knowledge files themselves
- Negative self-validation tests prove the scanner works: create temp bad reference, verify detection, clean up
- When addressing reviewer feedback, fix ALL mandatory items (1-3) before considering nice-to-haves (4-5) — but deliver all 5 if feasible
- git add -f is required for .squad/ files due to git exclude rules
- Team convention: use 'and' not ampersands in commit messages and documentation
(2026-03-16)
