📌 Team update (2026-03-07T14:16:00Z): CLI command wiring complete (agent-32) — Wired 4 missing commands to cli-entry.ts (rc, copilot-bridge, init-remote, rc-tunnel). Other 4 already present. 3,655 tests pass. PR #244 → dev. Pattern: Direct entry routing, no intermediate layers.
📌 Team update (2026-03-07T14:16:01Z): Model config reliability restored (agent-31) — Root cause: build pipeline emitted flat `**Model:**` but charter-compiler parsed `## Model` + `**Preferred:**`, losing preferences silently. Fix: AgentDefinition.model now `string | ModelPreference` with preferred/fallback/rationale. New defaults.model on SquadSDKConfig. Backwards compatible. 134 tests pass. PR #245 → dev. Pattern: Preserve intent through type, emit correct sections from charter generation.
